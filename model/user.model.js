...
const userSchema = new mongoose.Schema({
    ...
    userName: { type: String  ,  unique : [true, "User name is already exist"] ,required : true },
    email: { type: String  ,unique : true, required : true},
    encPassword: { type: String , required : true},
    profileImage : { type : String , required : true},
    dateOfBirth: { type: Date , required : true },
    subscriptionType: { type: String , enum : SubscriptionType , default : SubscriptionType.NOMAL },
    userRole: { type: String , enum : Role , default : Role.USER  },
    gender: { type: String ,  enum : Gender  , default : Gender.NOT_MENTIONED },
    mobile: [{ countryCode: { type: Number }, number: { type: Number }, isPrimary: { type: Boolean } }],
